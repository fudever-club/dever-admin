...
function ImageActivityManagementModule() {
  ...
  const handleUpload = async ({
    ...
    try {
      ...
      fmData.append("folder", "activities");
      ...
      message.success("Tải ảnh hoạt động lên Cloudflare R2 thành công!");
    } catch (err: any) {
      console.error("ImageActivity upload error:", err);
      setIsUploading(false);
      onError({ err });
      message.error(err?.response?.data?.message || err?.message || "Lỗi khi tải ảnh hoạt động!");
    }
  };
